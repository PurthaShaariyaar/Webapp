	...
	@RequestParam(name="operand2", required=false, defaultValue="") String operand2, Model model) {
		...
		switch(operator)
		{
			case "+":
				...
				return "result";
			case "|":
				model.addAttribute("result", Binary.orOperation(number1,number2).getValue());
				return "result";
			case "&":
				model.addAttribute("result", Binary.andOperation(number1,number2).getValue());
				return "result";
			case "*":
				model.addAttribute("result", Binary.multOperation(number1,number2).getValue());
				return "result";
			default:
				return "Error";
		}
